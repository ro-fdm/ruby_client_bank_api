...
module ShowMeTheMoney
	...
	def create_jim_with_bank_account
 	...
 	def create_emma_with_bank_accont
		...
		ap emma_bank_account
		emma_bank_account_id = emma_bank_account["id"]
		return emma_bank_account_id
 	end

 	def self.make_transfer(user_token, bank, origin_id, destination_id)
 		TransferAgent.new(user_token).calculate_payment( bank, origin_id,  destination_id, 2000000)
 	end
